...
document.getElementById('search').addEventListener('click', async function(evt){
    ...
    if (response.status === 'ok') {
        ...
        console.log(response.users.forEach(user => {console.log(user, user.media_name, user.num_entries)}))
        addItem("search_results", response.users.map(user => `Медіа: ${user.media_name}, кількість: ${user.num_entries}`));
    } else {
        document.getElementById('search_results').innerHTML = '';
