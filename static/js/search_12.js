...
document.getElementById('search').addEventListener('click', async function(evt){
    ...
    let response = await fetch(`/search_results_12?author_id=${authorID}&date_start=${dateStart}&date_end=${dateEnd}`);
    ...
    if (response.status === 'ok') {
        ...
        addItem("search_results", response.users.map(user => user.media_name+": "+user.orders_count));
    } else {
        document.getElementById('search_results').innerHTML = '';
